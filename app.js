...
mongoose.connection.on("connected", () => {
	console.log("Databaseye Bağlandım");
});

mongoose.connection.on("error", () => {
	console.error("Databaseye Bağlanamadım");
});
// </> DB Connection </>
...
app.get("/js", async (req, res) => {
  const codeData = require("./src/schemas/code");
  const data = await codeData.find({ rank: "js" }).sort({ date: -1 });
  res.render("js", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    data,
...
app.get("/jsplus", async (req, res) => {
  const codeData = require("./src/schemas/code");
  const data = await codeData.find({ rank: "jsplus" }).sort({ date: -1 });
  res.render("jsplus", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    data,
...
app.get("/altyapi", async (req, res) => {
  const codeData = require("./src/schemas/code");
  const data = await codeData.find({ rank: "altyapi" }).sort({ date: -1 });
  res.render("altyapi", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    data,
...
app.get("/altyapiplus", async (req, res) => {
  const codeData = require("./src/schemas/code");
  const data = await codeData.find({ rank: "altyapiplus" }).sort({ date: -1 });
  res.render("altyapiplus", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    data,
    moment,
...
app.get("/html", async (req, res) => {
  const codeData = require("./src/schemas/code");
  const data = await codeData.find({ rank: "html" }).sort({ date: -1 });
  res.render("html", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    data,
...
app.get("/diger", async (req, res) => {
  const codeData = require("./src/schemas/code");
  const data = await codeData.find({ rank: "diger" }).sort({ date: -1 });
  res.render("diger", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    data,
...
app.get("/topluluk", async (req, res) => {
  const codeData = require("./src/schemas/code");
  const data = await codeData.find({ rank: "topluluk" }).sort({ date: -1 });
  res.render("topluluk", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    data,
...
app.get("/js/:codeID", async (req, res) => {
  if (!req.user || !client.guilds.cache.get(conf.guildID).members.cache.has(req.user.id)) return error(res, 138, "Kodları Görebilmek İçin Discord Sunucumuza Katılmanız ve Siteye Giriş Yapmanız Gerekmektedir.");
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
   if (member && !member.roles.cache.has(conf.normalRole) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu Kodu Görebilmek İçin Gerekli Rolleriniz Bulunmamaktadır! Lütfen S.S.S Sayfasını Okuyunuz!");
  const codeID = req.params.codeID;
  if (!codeID) return res.redirect("/");
  const codeData = require("./src/schemas/code");
  const code = await codeData.findOne({ rank: "js", id: codeID });
...
app.get("/jsplus/:codeID", async (req, res) => {
  if (!req.user || !client.guilds.cache.get(conf.guildID).members.cache.has(req.user.id)) return error(res, 138, "Kodları Görebilmek İçin Discord Sunucumuza Katılmanız ve Siteye Giriş Yapmanız Gerekmektedir.");
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
  const codeID = req.params.codeID;
  if (!codeID) return res.redirect("/");
  if (member && !member.roles.cache.has(conf.goldRole) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu Kodu Görebilmek İçin Gerekli Rolleriniz Bulunmamaktadır! Lütfen S.S.S Sayfasını Okuyunuz!");
  const codeData = require("./src/schemas/code");
  const code = await codeData.findOne({ rank: "jsplus", id: codeID });
  if (!code) return error(res, 404, codeID+" ID'li bir kod bulunmuyor!");
  res.render("code", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
...
app.get("/bdfd/:codeID", async (req, res) => {
  ...
  const codeID = req.params.codeID;
  if (!codeID) return res.redirect("/");
  if (member && !member.roles.cache.has(conf.diaRole) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu Kodu Görebilmek İçin Gerekli Rolleriniz Bulunmamaktadır! Lütfen S.S.S Sayfasını Okuyunuz!");
  const codeData = require("./src/schemas/code");
...
app.get("/altyapi/:codeID", async (req, res) => {
  if (!req.user || !client.guilds.cache.get(conf.guildID).members.cache.has(req.user.id)) return error(res, 138, "Altyapıları Görebilmek İçin Discord Sunucumuza Katılmanız ve Siteye Giriş Yapmanız Gerekmektedir.");
  const guild = client.guilds.cache.get(conf.guildID);
  const member = guild.members.cache.get(req.user.id);
  const codeID = req.params.codeID;
  if (!codeID) return res.redirect("/");
  if (member && !member.roles.cache.has(conf.readySystemsRole) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu Altyapıyı Görebilmek İçin Gerekli Rolleriniz Bulunmamaktadır! Lütfen Bilgilendirme Sayfasını Okuyunuz!");
  const codeData = require("./src/schemas/code");
  const code = await codeData.findOne({ rank: "altyapi", id: codeID });
  if (!code) return error(res, 404, codeID+" ID'li bir kod bulunmuyor!");
  res.render("code", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
...
app.get("/altyapiplus/:codeID", async (req, res) => {
  if (!req.user || !client.guilds.cache.get(conf.guildID).members.cache.has(req.user.id)) return error(res, 138, "Altyapıları Görebilmek İçin Discord Sunucumuza Katılmanız ve Siteye Giriş Yapmanız Gerekmektedir.");
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
  const codeID = req.params.codeID;
  if (!codeID) return res.redirect("/");
  if (member && !member.roles.cache.has(conf.altyapiplusRole) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu Altyapıyı Görebilmek İçin Gerekli Rolleriniz Bulunmamaktadır! Lütfen Bilgilendirme Sayfasını Okuyunuz!");
  const codeData = require("./src/schemas/code");
  const code = await codeData.findOne({ rank: "altyapiplus", id: codeID });
  if (!code) return error(res, 404, codeID+" ID'li bir kod bulunmuyor!");
  res.render("code", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
...
app.get("/html/:codeID", async (req, res) => {
  if (!req.user || !client.guilds.cache.get(conf.guildID).members.cache.has(req.user.id)) return error(res, 138, "Kodları Görebilmek İçin Discord Sunucumuza Katılmanız ve Siteye Giriş Yapmanız Gerekmektedir.");
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
  const codeID = req.params.codeID;
  if (!codeID) return res.redirect("/");
  if (member && !member.roles.cache.has(conf.htmlRole) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu Kodu Görebilmek İçin Gerekli Rolleriniz Bulunmamaktadır! Lütfen S.S.S Sayfasını Okuyunuz!");
  const codeData = require("./src/schemas/code");
  const code = await codeData.findOne({ rank: "html", id: codeID });
  if (!code) return error(res, 404, codeID+" ID'li bir kod bulunmuyor!");
  res.render("code", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
...
app.get("/diger/:codeID", async (req, res) => {
  if (!req.user || !client.guilds.cache.get(conf.guildID).members.cache.has(req.user.id)) return error(res, 138, "Kodları Görebilmek İçin Discord Sunucumuza Katılmanız ve Siteye Giriş Yapmanız Gerekmektedir.");
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
  const codeID = req.params.codeID;
  if (!codeID) return res.redirect("/");
  if (member && !member.roles.cache.has(conf.digerRole) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu Kodu Görebilmek İçin Gerekli Rolleriniz Bulunmamaktadır! Lütfen S.S.S Sayfasını Okuyunuz!");
  const codeData = require("./src/schemas/code");
  const code = await codeData.findOne({ rank: "diger", id: codeID });
  if (!code) return error(res, 404, codeID+" ID'li bir kod bulunmuyor!");
  res.render("code", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
...
app.get("/topluluk/:codeID", async (req, res) => {
  ...
  if (!codeID) return res.redirect("/");
  const codeData = require("./src/schemas/code");
  const code = await codeData.findOne({ rank: "topluluk", id: codeID });
  if (!code) return error(res, 404, codeID+" ID'li bir kod bulunmuyor!");
  res.render("code", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
